...
const CalendarPreview = ({ startDate, endDate, recurrence }) => {
  const generateRecurringDates = () => {
    ...
    // Simplified day of the week mapping
    const daysOfWeek = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
    const specificDayIndices = specificDays.map(day => daysOfWeek.indexOf(day));
    ...
    while (!endDateObj || currentDate <= endDateObj) {
      ...
      if (frequency === 'weekly') {
        ...
        if (specificDays.length === 0) {
          ...
          const specificDayIndex = daysOfWeek.indexOf(specificDay);
